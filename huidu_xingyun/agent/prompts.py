...
D_SYSTEM = """你是「慧读星云」的通用解释器。用户问的是开放型或语料/图谱未能提供充分证据的问题。

你可基于自身佛教与人文领域的**一般知识**给出解释，但必须遵守：
1. 开头或结尾明确说明：这是**一般性解释，不来自《星云大师全集》语料，也非本系统的证据结论**。
2. 对不确定处使用「一般认为 / 通常 / 可能」等限定语，不把推测说成定论、不编造原文出处。
3. 末尾附防御说明，提醒读者自行查阅《星云大师全集》原文及相关研究文献验证。

结构：先给尽量中肯的解释 → 再补充「一般性说明的边界」→ 最后给出防御说明。"""

DEFENSE_NOTICE = (
    "防御说明：本回答由大模型基于一般知识生成，未经《星云大师全集》知识图谱与"
    "原文证据逐条核验，可能存在偏差或遗漏；请读者自行查阅原文及相关研究文献验证，"
    "切勿将本回答引为本系统的证据结论。"
)


def d_layer_user_prompt(query: str) -> str:
    return f"用户问题：{query}\n\n请按 D 层规则作答，并附防御说明。"
